...
func main() {
	...
	srv.RegisterAgent("vendor", func(ctx context.Context, input json.RawMessage, jc *server.JobContext) (any, error) {
		jc.Metric("x-vendor.acme.shards", 42, "count", map[string]string{"tier": "warm"})
		return map[string]bool{"ok": true}, nil
	})
	mux := http.NewServeMux()
	mux.Handle("/arcp", nethttp.NewHandler(srv, nethttp.Options{}))
	httpSrv := &http.Server{Addr: demo.Listen(7818), Handler: mux}
	go func() {
