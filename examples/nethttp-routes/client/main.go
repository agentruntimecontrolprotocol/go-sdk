...
func main() {
	...
	defer cancel()
	t, err := transport.DialWebSocket(ctx, demo.Addr(7830), transport.WebSocketOptions{})
	demo.Must(err)
	cli, err := client.Connect(ctx, t, client.Options{})
	demo.Must(err)
	defer cli.Close(ctx)
	h, err := cli.Submit(ctx, client.SubmitRequest{Agent: "echo", Input: map[string]string{"hi": "there"}})
	demo.Must(err)
	res, err := h.Wait(ctx)
	demo.Must(err)
